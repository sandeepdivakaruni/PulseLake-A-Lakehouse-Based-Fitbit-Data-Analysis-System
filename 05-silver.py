...
class Silver():
    ...
    def upsert_users(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        df_delta = (spark.readStream
                         ...
                         .withWatermark("registration_timestamp", "365 days")
    ...
    def upsert_user_profile(self, once=False, processing_time="15 seconds", startingVersion=0):
        ...
        df_cdc = (spark.readStream
                       ...
                       .select("user_id", F.to_date('dob','MM/dd/yyyy').alias('dob'),
                               ...
                               "update_type")
                       .withWatermark("updated", "365 days")
    ...
    def upsert_completed_workouts(self, once=True, processing_time="15 seconds", startingVersion=0):
        ...
        ON a.user_id=b.user_id AND a.workout_id = b.workout_id AND a.session_id=b.session_id AND a.start_time = b.start_time
        WHEN NOT MATCHED THEN INSERT *
        """
        ...
        df_start = (spark.readStream
                         ...
                         .withWatermark("start_time", "365 days")
        ...
        df_stop = (spark.readStream
                         ...
                         .withWatermark("end_time", "365 days")
        ...
        #               - stop < start + 3 hours
        join_condition = [df_start.user_id == df_stop.user_id, df_start.workout_id==df_stop.workout_id, df_start.session_id==df_stop.session_id, df_stop.end_time > df_start.start_time,
